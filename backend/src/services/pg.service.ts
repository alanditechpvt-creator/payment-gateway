  ...
  async createPG(data: CreatePGDTO) {
    const pg = await prisma.paymentGateway.create({
      data: {
        ...
        supportedTypes: data.supportedTypes || ['PAYIN', 'PAYOUT'],
      },
    });
  ...
  async updatePG(pgId: string, data: Partial<CreatePGDTO> & { isActive?: boolean }) {
    const pg = await prisma.paymentGateway.update({
      ...
      data: {
        ...
        baseRate: data.baseRate ? new Decimal(data.baseRate) : undefined,
        minAmount: data.minAmount !== undefined ? (data.minAmount ? new Decimal(data.minAmount) : null) : undefined,
        maxAmount: data.maxAmount !== undefined ? (data.maxAmount ? new Decimal(data.maxAmount) : null) : undefined,
